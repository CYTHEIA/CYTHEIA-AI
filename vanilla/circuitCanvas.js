...
export function createCircuitCanvas() {
  ...
  container.className = 'flex-1 bg-[#0a0a0c] overflow-hidden relative';
...
export function renderCircuitCanvas(container) {
  ...
  for (const component of state.project.components) {
    ...
    rect.setAttribute('y', -def.height / 2);
    ...
    text.textContent = def.label.substring(0, 10);
    g.appendChild(text);
    
    // Draw pins
    for (const pin of def.pins) {
      const circle = document.createElementNS(SVG_NS, 'circle');
      circle.setAttribute('cx', pin.x - def.width / 2);
      circle.setAttribute('cy', pin.y - def.height / 2);
