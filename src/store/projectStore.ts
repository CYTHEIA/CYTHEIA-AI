...
interface ProjectStore {
  ...
  addComponent: (type: string, x: number, y: number) => ID | null;
  removeComponent: (id: ID) => void;
  removeComponents: (ids: ID[]) => void;
  moveComponent: (id: ID, x: number, y: number) => void;
  rotateComponent: (id: ID) => void;
  duplicateComponent: (id: ID) => ID | null;
  setComponentProperty: (id: ID, key: string, value: any) => void;
  ...
  updateCode: (index: number, content: string) => void;
  addCodeFile: (name: string) => void;
...
export const useProjectStore = create<ProjectStore>((set, get) => ({
  ...
  duplicateComponent: (id) => {
    ...
    if (!comp) return null;
    const newId = uid();
    const dup: PlacedComponent = { ...comp, id: newId, x: comp.x + 40, y: comp.y + 40, props: { ...comp.props } };
